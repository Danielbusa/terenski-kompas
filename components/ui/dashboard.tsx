...
function StatCard({ icon, label, value, trend, tone = "success", className }: { icon: React.ReactNode; label: React.ReactNode; value: React.ReactNode; trend?: React.ReactNode; tone?: "success" | "warning" | "danger" | "neutral"; className?: string }) {
  ...
  return <Card data-slot="stat-card" className={cn("relative min-h-32 gap-5 overflow-hidden p-4 py-4", className)}><div className="flex items-center gap-2.5 text-sm font-semibold text-foreground"><span className="grid size-9 place-items-center rounded-lg border bg-card text-muted-foreground [&_svg]:size-4">{icon}</span>{label}</div><div className="flex items-end justify-between gap-3"><strong className="text-3xl font-bold tracking-tight text-foreground">{value}</strong>{trend ? <span className={cn("max-w-[60%] truncate rounded-full px-2.5 py-1 text-xs font-semibold", toneClass)}>{trend}</span> : null}</div><span aria-hidden className="pointer-events-none absolute -right-3 -bottom-4 text-border/60 [&_svg]:size-20 [&_svg]:stroke-1">{icon}</span></Card>;
}
