...
function SectionCard({ title, eyebrow, description, action, icon, className, contentClassName, children }: { title?: React.ReactNode; eyebrow?: React.ReactNode; description?: React.ReactNode; action?: React.ReactNode; icon?: React.ReactNode; className?: string; contentClassName?: string; children: React.ReactNode }) {
  return <Card data-slot="section-card" className={cn("gap-0 overflow-hidden py-0", className)}>{title || eyebrow || description || action || icon ? <CardHeader className="grid min-h-16 grid-cols-[1fr_auto] items-center gap-3 border-b px-5 py-4"><div>{eyebrow ? <p className="text-xs font-semibold uppercase tracking-[.1em] text-muted-foreground">{eyebrow}</p> : null}{title ? <CardTitle className={cn("text-base", eyebrow && "mt-1")}>{title}</CardTitle> : null}{description ? <p className="mt-1 text-xs font-medium text-muted-foreground">{description}</p> : null}</div>{action ?? <span className="text-muted-foreground [&_svg]:size-5">{icon}</span>}</CardHeader> : null}<CardContent className={cn("px-0", contentClassName)}>{children}</CardContent></Card>;
}
