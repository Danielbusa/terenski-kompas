...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  if (!isSupabaseConfigured) {
    return <main className="setup-screen"><div className="setup-card"><span className="brand-mark"><Compass /></span><p className="eyebrow">{t("POTREBNO POVEZIVANJE", "CONNECTION REQUIRED")}</p><h1>{t("Dashboard čeka Supabase podatke", "The dashboard is waiting for Supabase")}</h1><p>{t("Veza sa bazom nije podešena.", "The database connection is not configured.")}</p><div><a href="/join">{t("Prijava volontera", "Volunteer signup")}</a><a href="/donate">{t("Donacije", "Donations")}</a></div></div></main>;
  }
