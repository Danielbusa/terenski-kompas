...
export default function DonatePage() {
  ...
  const reference = useMemo(() => `${defaultPaymentConfig.referencePrefix}-${new Date().toISOString().slice(0, 10).replaceAll("-", "")}`, []);
  const payload = useMemo(() => createIpsPayload(Math.max(1, amount || 0), defaultPaymentConfig, reference), [amount, reference]);
  useEffect(() => { void QRCode.toDataURL(payload, { errorCorrectionLevel: "M", margin: 2, width: 360 }).then(setQr); }, [payload]);
