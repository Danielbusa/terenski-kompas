...
import { defaultPaymentConfig, createIpsPayload, type PaymentConfig } from "@/lib/payments";
...
export default function DonatePage() {
  ...
  const reference = `${config.referencePrefix}-DONACIJA`;
  const payload = useMemo(() => createIpsPayload(Math.max(1, amount || 0), config, reference), [amount, config, reference]);
  useEffect(() => { void (async () => { const supabase = getSupabase(); if (!supabase) return; const { data } = await supabase.from("app_config").select("value").eq("key", "nbs_ips").maybeSingle(); if (data?.value) setConfig({ ...defaultPaymentConfig, ...(data.value as Partial<PaymentConfig>) }); })(); }, []);
  useEffect(() => {
    let active = true;
    void import("qrcode")
