...
import { AlertTriangle, ArrowRight, BarChart3, CalendarCheck, CheckCircle2, ClipboardList, CloudOff, LayoutDashboard, LoaderCircle, LocateFixed, Map, MapPin, MapPinned, Navigation, Plus, Radio, Receipt, RefreshCw, Search, Settings, ShieldCheck, Signal, Trophy, UserCheck, UserRound, Users, WifiOff } from "lucide-react";
...
function OperationsDashboard() {
  ...
  return <div className={`ops-shell ${railCollapsed ? "rail-collapsed" : ""}`}>
    <AppSidebar collapsed={railCollapsed} onToggle={() => setRailCollapsed((value) => !value)} onSignOut={() => void signOut()} name={profile.full_name} role={roleLabel(profile.role, language)} initials={initials(profile.full_name)} items={[{ label: t("Kontrolna tabla", "Dashboard"), icon: <LayoutDashboard />, active: workspace === "field", onClick: () => setWorkspace("field") }, ...visibleNav.filter((item) => item.id !== "field").map((item) => ({ label: item.label, icon: item.icon, active: workspace === item.id, onClick: () => setWorkspace(item.id) })), ...(["admin","coordinator"].includes(profile.role) ? [{ label: t("Administracija", "Admin"), icon: <BarChart3 />, href: "/admin" }] : []), { label: t("Profil", "Profile"), icon: <Settings />, href: "/profile" }]} />
    <main className="ops-main">
      <header className="ops-header">
        <div className="ops-brand"><BrandMark /><div><b>{workspace === "field" ? t("Terenska kontrolna tabla", "Field dashboard") : workspaceTitle(workspace, t)}</b><small>{workspace === "field" ? t("Pregled terenskih aktivnosti širom Srbije uživo", "Live view of canvassing activity across Serbia") : workspaceDescription(workspace, t)}</small></div></div>
...
function FieldWorkspace({ data, pending, profile, onSaved, onVisit }: { data: DashboardData; pending: number; profile: Profile; onSaved: () => void; onVisit: (task: FieldTask | null) => void }) {
  const { t, language } = useLanguage();
  const [focusedVisit, setFocusedVisit] = useState<VisitMarker | null>(null);
  ...
  return <div className="workspace-stack">
    ...
    <div className="field-layout reference-field-layout"><SectionCard className="reference-map-card" title={t("Mapa stanica turneje", "Tour stops map")} description={t("Pinovi su obojeni prema ishodu posete", "Pins are coloured by visit outcome")}><LiveFieldMap compact focus={focusedVisit} tasks={data.tasks} visits={data.visits} tourStops={data.tourStops} onSelectTask={onVisit} /></SectionCard><SectionCard className="visit-entry-card" title={t("Zabeleži posetu", "Log a visit")} description={t("Pretraga adrese, GPS i ishod na jednom mestu", "Address search, GPS and outcome in one place")}><VisitEntryForm task={null} profile={profile} onSaved={onSaved} /></SectionCard></div>
    <SectionCard className="recent-visits-card" title={t("Nedavne posete", "Recent visits")} description={t("Najnoviji izveštaji sa terena", "Latest field reports from the team")}><div className="recent-visits-table"><table><thead><tr><th>{t("Adresa", "Address")}</th><th>{t("Grad", "City")}</th><th>{t("Ishod", "Outcome")}</th><th>{t("Terenski radnik", "Canvasser")}</th><th>{t("Vreme", "When")}</th></tr></thead><tbody>{data.visits.slice(0, 8).map((visit) => <tr key={visit.id} tabIndex={0} onClick={() => setFocusedVisit(visit)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setFocusedVisit(visit); }}><td>{visit.address || t("GPS lokacija", "GPS location")}</td><td>{visit.city_village || profile.assigned_region || "—"}</td><td><StatusBadge tone={visit.status === "visited_supporter" ? "success" : visit.status === "visited_hostile" || visit.status === "refused" ? "danger" : "warning"}>{visitOutcome(visit.status, language)}</StatusBadge></td><td>{profile.full_name}</td><td>{new Intl.DateTimeFormat(language === "sr" ? "sr-RS" : "en-GB", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }).format(new Date(visit.completed_at))}</td></tr>)}</tbody></table>{data.visits.length === 0 && <EmptyState icon={<MapPin />} title={t("Još nema poseta", "No visits yet")} text={t("Prva evidentirana poseta pojaviće se ovde.", "Your first logged visit will appear here.")} />}</div></SectionCard>
  </div>;
}
